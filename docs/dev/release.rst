...
This page gives an overview of how Squarebot releases are made.
...
Squarebot follows semver_, so follow its rules to pick the next version:
...
The tag **must** follow the :pep:`440` specification since Squarebot uses setuptools_scm_ to set version metadata based on Git tags.
